...
def dirac_conv2d(inputconv, output_dim=64, filter_height=5, filter_width=5, stride_height=1, stride_width=1,
    ...
    with tf.variable_scope(name) as scope:
        ...
        output_conv = tf.nn.conv2d(inputconv, alpha * dirac_weight + beta * weight, [1, stride_height, stride_width, 1],
                                   padding=padding)
        output_conv=tf.nn.bias_add(output_conv, bias)
        output_conv = tf.nn.relu(output_conv)
...
def dirac1_conv2d(inputconv, output_dim=64, filter_height=5, filter_width=5, stride_height=1, stride_width=1,
                  stddev=0.02,padding="SAME", do_norm=True, norm_type='batch_norm', name="dirac1_conv2d"):
    with tf.variable_scope(name) as scope:
        ...
        #output_conv = tf.nn.conv2d(inputconv, alpha * dirac_weight + beta, [1, stride_height, stride_width, 1],
         #                          padding=padding)
        output_conv = tf.nn.conv2d(inputconv, alpha + beta * weight, [1, stride_height, stride_width, 1],
                                   padding=padding)
